...
#------处理表达数据------------------------------------------------------------
sample_data <- read.xlsx(data_file) %>% names() %>% .[-1] %>% tolower() %>% trimws() %>% data.frame()
colnames(sample_data) <- "sample"

sample_trait <- read.xlsx(expriment_info) %>% .[,1]%>% tolower()%>% trimws()%>% data.frame()
colnames(sample_trait) <- "sample"

sm <- anti_join(sample_data,sample_trait,by = "sample") %>% as.character()

if(!sm=="character(0)")
{print(paste0("There are differnt smaples between the data_file and expriment_info: ",sm))
stop("Please check the sample in the data_file and expriment_info.")}
...
par(cex = 0.6);
par(mar = c(0,4,2,0))
png(filename = "analysis results/1.Sample clustering 01.png",
...
#---------------删除离群样本--------------------
if(!exists("abnormal_sample_height",where = -1))
  abnormal_sample_height = NA

if(!is.na(abnormal_sample_height)){

sizeGrWindow(12,9) #视图
par(cex = 0.6);
par(mar = c(0,4,2,0))
png(filename = "analysis results/1.Sample clustering 02.png",
    width=800, height=600,units = "px",res = 100)
par(mai = c(1, 1, 1, 1)) # 边距 c(bottom, left, top, right)
...
# ----boxplot.stats 筛选出异常数据---------------
abl_val <- boxplot.stats(sampleTree$height)$out %>% as.numeric() %>% sort(decreasing = T)

abline(h = abnormal_sample_height, col = "red") #划定需要剪切的枝长
clust = cutreeStatic(sampleTree, cutHeight = abnormal_sample_height, minSize = 10)
#这时候会从高度为15这里横切，把离群样本分开
table(clust)
keepSamples = (clust==1)  #保留非离群(clust==1)的样本
datExpr = datExpr[keepSamples, ]  #去除离群值后的数据
sampleTree = hclust(dist(datExpr), method = "average")
nGenes = ncol(datExpr)
...
par(cex = 0.6);
par(mar = c(0,4,2,0))
png(filename = "analysis results/1.Sample clustering 03.png",
    width=800, height=600,units = "px",res = 100)
par(mai = c(1, 1, 1, 1)) # 边距 c(bottom, left, top, right)
plot(sampleTree, main = "Sample clustering to detect outliers", sub="", xlab="", cex.lab = 1.5,
     cex.axis = 1.5, cex.main = 2)
...
# -----------------载入表型数据-----------------
traitData0 = read.xlsx(expriment_info,rowNames = T) %>% data.frame()

sample <- rownames(datExpr)

sample_n <- match(sample,rownames(traitData0))

traitData <- traitData0[sample_n,] %>% data.frame()
rownames(traitData) <- sample
colnames(traitData) <- colnames(traitData0)
...
png(filename = "analysis results/7.Module-trait relationships.png",
    width=1000, height=high,units = "px",res = 100)
par(mai = c(1, 2, 1, 1)) # 边距 c(bottom, left, top, right)
